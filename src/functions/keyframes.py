...
def find_next_key_frame(idx1, idx2):
    ...
    :returns: list of dict with structure [{'startPoint': 2DPoint, 'endPoint': 2DPoint}],
    ...
    while success and curr_idx < idx2:
        ...
        # trace
        match_points_1, match_points_2, matches = \
            matcher_functions.detect_and_match_keypoints(prev_frame, frame, detector=models.Detector.SURF)
...
def get_all_keyframes(start_frame_idx, end_frame_idx):
    ...
    tmp_kf, keyframe_id = find_next_key_frame(keyframe_idx[-2], end_frame_idx)
    keyframes = keyframes + tmp_kf
    start_idx.append(tmp_kf[0][0]['start_frame'])
